...
def parse_args():
    ...
    parser.add_argument('--Tree_Agent', type=lambda x: bool(strtobool(x)), default=False, nargs='?', const=True,
        help='if toggled, the agent will use the Tree_Agent model instead of the standard PPO model')
...
class Tree_Agent(nn.Module):
    def __init__(self, envs, mapsize=16 * 16, statistic_dims = [0]):
        super(Tree_Agent, self).__init__()
        self.mapsize = mapsize
        h, w, c = envs.observation_space.shape
        ...
        self.encoder_old = nn.Sequential(
            Transpose((0, 3, 1, 2)), # Transpose to (batch, channels, height, width)
            layer_init(nn.Conv2d(c, 64, kernel_size=3, padding=1)), 
            nn.MaxPool2d(3, stride=2, padding=1),
            nn.ReLU(),
            layer_init(nn.Conv2d(64, 64, kernel_size=3, padding=1)), 
            nn.MaxPool2d(3, stride=2, padding=1),
        ...
        self.statistic_size = statistic_dims
        self.register_buffer("mask_value", torch.tensor(-1e8))
...
if __name__ == "__main__":
    ...
    coac_envs = (args.num_bot_envs-6) // 3
    tiamat_envs = (args.num_bot_envs-6) // 3
    ...
    envs = MicroRTSGridModeVecEnv(
        ...
        max_steps=2000,
        render_theme=2,
        ai2s=[microrts_ai.coacAI for _ in range(coac_envs)]
        + [microrts_ai.randomBiasedAI for _ in range(random_biased_envs)]
        + [microrts_ai.droplet for _ in range(droplet_envs)]
