...
def evaluate(our_tree, enemy_tree, T_rush):
  scores = {}
  curr = our_tree.root
  ...
  def traverse_enemy(leaf, node):
    if node == None:
      return

    if node.time_till < (leaf.time_till - T_rush):
        if scores[leaf] == None: #cahgned to 0, from None
          scores[leaf] = ((leaf.military_strength - node.military_strength) + leaf.state_vector[0])
        else:
          scores[leaf] = min(scores[leaf], ((leaf.military_strength - node.military_strength) + leaf.state_vector[0]))
    for kid in node.children:
      traverse_enemy(leaf, kid)


  traversal(curr)
  print(scores.keys())
  for leaf in scores.keys():
    traverse_enemy(leaf, enemy_tree.root)

  if not scores.keys():
    return None
  return max(scores.keys()) #i want to return the top 3, not just the biggest
...
def executeTwoTrees(inputs_for_both_trees):
  ...
  output_vector = get_action_recommendation(best_leaf_recommendations)  #convert to tensor, where the first index is the batch index
  output_tensor = torch.tensor(output_vector, dtype=torch.float32)

  print(output_tensor) #should return
...
def bigBatch(tree_input):
  ...
  for i in range(batch_num):
    ...
    inputs_for_both_trees = [[scalars1[i,:], worker_map1[i,:,:], barracks_map1[i,:,:], resource_map1[i,:,:], base_map1[i,:,:]], [scalars2[i,:], worker_map2[i,:,:], barracks_map2[i,:,:], resource_map2[i,:,:], base_map2[i,:,:]]]
    executeTwoTrees(inputs_for_both_trees)
